...
// @namespace    https://github.com/richmondgoh8
// @author       richmondgoh8
// @license      MIT
// @version      7.1.24
...
// @grant        unsafeWindow
// @homepageURL  https://github.com/richmondgoh8/Steam-Economy-Enhancer
// @homepage     https://github.com/richmondgoh8/Steam-Economy-Enhancer
// @supportURL   https://github.com/richmondgoh8/Steam-Economy-Enhancer/issues
// @downloadURL  https://raw.githubusercontent.com/richmondgoh8/Steam-Economy-Enhancer/master/code.user.js
// @updateURL    https://raw.githubusercontent.com/richmondgoh8/Steam-Economy-Enhancer/master/code.user.js
// ==/UserScript==
...
(function ($, async) {
  ...
  function request(url, options, callback) {
    ...
    if (request.stopped) {
      const error = new Error(
        'Request was not sent to the server, something went wrong',
      );
  ...
  function formatPrice(valueInCents) {
    return unsafeWindow.v_currencyformat(
      valueInCents,
      currencyCode,
      currencyCountry,
    );
  ...
  function getPriceInformation(isTradingCard, isFoilTradingCard) {
    ...
    let minPriceBeforeFees = market.getPriceBeforeFees(minPrice);

    let minSellerPrice = 1;
    if (currencyCode === 'CNY') {
      minSellerPrice = 7;
    }
    minPriceBeforeFees = Math.max(minPriceBeforeFees, minSellerPrice);
  ...
  function calculateListingPriceBeforeFees(histogram) {
    ...
    const shouldIgnoreLowestListingOnLowQuantity =
    ...
    if (
      ...
      const listingPrice2ndLowest = market.getPriceBeforeFees(
  ...
  function calculateSellPriceBeforeFees(
    ...
    if (
      ...
      const buyOrderPrice = market.getPriceBeforeFees(
  ...
  SteamMarket.prototype.getCurrentItemOrdersHistogram = function (
    ...
    market.getMarketItemNameId(item, (error, item_nameid) => {
      if (error) {
        callback(ERROR_FAILED);
        return;
      }
  ...
  SteamMarket.prototype.getPriceBeforeFees = function (price, item) {
    ...
    if (item != null) {
      if (item.market_fee != null) {
        ...
        item.description.market_fee != null
      ) {
  ...
  SteamMarket.prototype.getPriceIncludingFees = function (price, item) {
    ...
    price = Math.max(Math.round(price), minSellerPrice);
  ...
  function getIsFoilTradingCard(item) {
    ...
    if (tags != null) {
      ...
      tags.forEach((arrayItem) => {
        if (
          arrayItem.category == 'cardborder' &&
          arrayItem.internal_name == 'cardborder_1'
        ) {
          // foil border.
  ...
  function CalculateAmountToSendForDesiredReceivedAmount(
    ...
    let minFee = walletInfo['wallet_fee_minimum'] || 1;

    if (currencyCode === 'CNY') {
      minFee = Math.max(minFee, 7);
    }
    ...
    const nPublisherFee =
      publisherFee > 0
        ? Math.max(parseInt(roundFee(receivedAmount * publisherFee)), minFee)
        : 0;
  ...
  if (currentPage == PAGE_INVENTORY) {
    ...
    const sellQueue = async.queue((task, next) => {
      ...
      if (
        ...
        logDOM(
          `${padLeft} - ${itemNameWithAmount} is not listed due to ignoring price settings.`,
        );
        $(`#${task.item.appid}_${task.item.contextid}_${itemId}`).css(
          'background',
    ...
    function scrapQueueWorker(item, callback) {
      ...
      market.getGooValue(item, (err, goo) => {
        ...
        if (err != ERROR_SUCCESS) {
          ...
          logDOM(
            `${padLeft} - ${itemName} not turned into gems due to missing gems value.`,
          );
        ...
        market.grindIntoGoo(item, (err) => {
          if (err != ERROR_SUCCESS) {
            logConsole(`Failed to turn item into gems for ${itemName}`);
            logDOM(
              `${padLeft} - ${itemName} not turned into gems due to unknown error.`,
            );
          ...
          logDOM(
            `${padLeft} - ${itemName} turned into ${item.goo_value_expected} gems.`,
          );
          $(`#${item.appid}_${item.contextid}_${itemId}`).css(
            'background',
            COLOR_SUCCESS,
    ...
    function sellSelectedItemsManually() {
      getInventorySelectedMarketableItems((items) => {
        ...
        const dialog = unsafeWindow.ShowDialog(
    ...
    const itemQueue = async.queue((item, next) => {
      itemQueueWorker(item, item.ignoreErrors, (success, cached) => {
        if (success) {
          ...
          numberOfFailedRequests =
            numberOfFailedRequests > 3 ? 0 : numberOfFailedRequests;
    ...
    function initializeInventoryUI() {
      ...
      $('#inventories').selectable({
        ...
        selecting: function (e, ui) {
          ...
          const selectedIndex = $(ui.selecting.tagName, e.target).index(
            ui.selecting,
          );
    ...
    async function updateInventorySelection(selectedItem) {
      ...
      while (
        timeDelayed < 2500 &&
        item_info.find('a[href^="https://steamcommunity.com/market/listings/"]')
          .length == 0
      ) {
        await delay(100);
        timeDelayed += 100;
      ...
      const isBoosterPack = selectedItem.name
        .toLowerCase()
      ...
      market.getItemOrdersHistogram(item, false, (err, histogram) => {
        if (err) {
          logConsole(
            `Failed to get orders histogram for ${selectedItem.name || selectedItem.description.name}`,
          );
          return;
        }
    ...
    function updateInventoryUI(isOwnInventory) {
      ...
      if (isOwnInventory) {
        ...
        $('.unpack_selected_booster_packs').on(
          'click',
          '*',
          unpackSelectedBoosterPacks,
        );
      }
      ...
      loadAllInventories().then(() => {
        let updateInventoryPricesTimer;
        const updateInventoryPrices = function () {
          clearTimeout(updateInventoryPricesTimer);
          updateInventoryPricesTimer = setTimeout(() => {
    ...
    async function loadAllInventories() {
      ...
      for (const inventory of [...childs, main]) {
        await new Promise((resolve) =>
          inventory.LoadCompleteInventory().done(resolve),
        );
      }
    }
    ...
    function getInventoryItems() {
      ...
      for (const child in getActiveInventory().m_rgChildInventories) {
        for (const key in getActiveInventory().m_rgChildInventories[child]
          .m_rgAssets) {
          const value =
  ...
  if (currentPage == PAGE_INVENTORY || currentPage == PAGE_TRADEOFFER) {
    ...
    const inventoryPriceQueue = async.queue((item, next) => {
      inventoryPriceQueueWorker(item, false, (success, cached) => {
        if (success) {
          setTimeout(() => next(), cached ? 0 : getRandomInt(1000, 1500));
        } else {
          if (!item.ignoreErrors) {
    ...
    function inventoryPriceQueueWorker(item, ignoreErrors, callback) {
      ...
      market.getItemOrdersHistogram(
        ...
        (err, histogram, cachedListings) => {
          ...
          element.append(
            `<span class="inventory_item_price price_${sellPrice == 65535 ? 0 : market.getPriceIncludingFees(sellPrice)}">${itemPrice}</span>`,
          );
  ...
  if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
    ...
    function marketListingsQueueWorker(listing, ignoreErrors, callback) {
      ...
      if (listingUI == null) {
        logConsole(
          `Listing ${listing.listingid} not found in the lists, skipping.`,
        );
              ...
              unsafeWindow.RequestFullInventory(
                ...
                (transport) => {
                  if (
                    ...
                    transport.responseJSON.success
                  ) {
    ...
    function fillMarketListingsQueue() {
      ...
      marketLists
        ...
        .forEach((item) => {
          const isBuyOrder =
            item.elm.id.startsWith('mbuyorder_') ||
            item.elm.id.startsWith('mybuyorder_');
          const isSellOrder = item.elm.id.startsWith('mylisting_');

          if (isSellOrder) {
            const listingid = replaceNonNumbers(
              item.values().market_listing_item_name,
            );
            const assetInfo = getAssetInfoFromListingId(listingid);
            ...
            if (!isNaN(assetInfo.priceBuyer)) {
              totalSellOrderPriceBuyer +=
                assetInfo.priceBuyer * assetInfo.amount;
            }
            if (!isNaN(assetInfo.priceSeller)) {
              totalSellOrderPriceSeller +=
                assetInfo.priceSeller * assetInfo.amount;
            }
      ...
      $('#my_market_selllistings_number')
        .append(
          `<span id="my_market_sell_listings_total_amount"> [${totalSellOrderAmount}]</span>`,
        )
        .append(
      ...
      $('#my_market_buylistings_number')
        .append(
          `<span id="my_market_buy_listings_total_amount"> [${totalBuyOrderAmount}]</span>`,
        )
        .append(
    ...
    function getAssetInfoFromListingId(listingid) {
      ...
      const actionButton = $('.item_market_action_button', listing.elm).attr(
        'href',
      );
      ...
      const amount = Number(
        unsafeWindow.g_rgAssets[appid][contextid][assetid]?.amount ?? 1,
      );
      return {
        appid,
        contextid,
    ...
    function getAssetInfoFromBuyOrderId(orderid) {
      ...
      if (
        !listing.elm.id.startsWith('mbuyorder_') &&
        !listing.elm.id.startsWith('mybuyorder_')
      ) {
        return {};
      }

      const amount = parseInt(
        $('.market_listing_buyorder_qty', listing.elm).text().trim(),
      );
      const price = getPriceValueAsInt(
        $('.market_listing_price', listing.elm)[0].innerText,
      );
      ...
      if (currentPage == PAGE_MARKET) {
        ...
        $('.market_home_listing_table').each(function () {
          ...
          $(this)
            .children()
            .last()
            .wrap('<div class="market_listing_see"></div>');
          const marketListing = $('.market_listing_see', this).last();
          ...
          function () {
            ...
            for (const appid in unsafeWindow.g_rgAssets) {
              for (const contextid in unsafeWindow.g_rgAssets[appid]) {
                for (const assetid in unsafeWindow.g_rgAssets[appid][
                  contextid
                ]) {
                  existingAsset =
            ...
            unsafeWindow.g_rgAssets[assetInfo.appid][assetInfo.contextid][
              assetInfo.assetid
            ] = existingAsset;
            marketListingsQueue.push({
              listingid,
    ...
    function sortMarketListings(elem, isPrice, isDateOrQuantity, isName) {
      ...
      const isBuyOrder =
        list.list.querySelectorAll('.market_listing_buyorder_qty').length >= 1;
      ...
      if (isName) {
        ...
        if (isBuyOrder) {
          list.sort('market_listing_buyorder_qty', {
            order: asc ? 'asc' : 'desc',
            sortFunction: function (a, b) {
              const quantityA = a.elm.querySelector(
                '.market_listing_buyorder_qty',
              ).innerText;
              const quantityB = b.elm.querySelector(
                '.market_listing_buyorder_qty',
              ).innerText;
        ...
        list.sort('market_listing_price', {
          ...
          sortFunction: function (a, b) {
            if (!isBuyOrder) {
              ...
              listingPriceA = listingPriceA.substr(
                0,
                listingPriceA.indexOf('('),
              );

              let listingPriceB = $(b.values().market_listing_price).text();
              listingPriceB = listingPriceB.substr(
                0,
                listingPriceB.indexOf('('),
              );
    ...
    function initializeMarketUI() {
      $('.market_header_text').append(
        '<progress id="see_market_progress" value="1" max="1" hidden>',
      );
      marketProgressBar = document.getElementById('see_market_progress');

      // Sell orders.
      $('.my_market_header').first()
        .append(`<div class="market_listing_buttons">
  ...
  if (currentPage == PAGE_TRADEOFFER) {
    ...
    function getTradeOfferInventoryItems() {
      ...
      for (const child in activeInventory.rgChildInventories) {
        for (const key in activeInventory.rgChildInventories[child]
          .rgInventory) {
          const value =
            activeInventory.rgChildInventories[child].rgInventory[key];
          if (typeof value === 'object') {
            // Merges the description in the normal object, this is done to keep the layout consistent with the market page, which is also flattened.
            Object.assign(value, value.description);
            // Includes the id of the inventory item.
  ...
  function hasLoadedAllTradeOfferItems() {
    for (
      ...
      const asset = unsafeWindow.UserThem.findAsset(
    ...
    for (
      ...
      const asset = unsafeWindow.UserYou.findAsset(
  ...
  function initializeTradeOfferUI() {
    if (getSettingWithDefault(SETTING_TRADEOFFER_PRICE_LABELS) == 1) {
      ...
      $('.trade_right > div > div > div > .trade_item_box').observe(
        ...
        () => {
          ...
          const your_sum = sumTradeOfferAssets(
            unsafeWindow.g_rgCurrentTradeStatus.me.assets,
            unsafeWindow.UserYou,
          );
          const their_sum = sumTradeOfferAssets(
    ...
    if (
      location.pathname !== '/tradeoffer/new/' &&
      location.pathname !== '/tradeoffer/new'
    ) {
    ...
    unsafeWindow
      ...
      .done(() => {
        ...
        setSetting(
          ...
          $(`#${SETTING_QUICK_SELL_BUTTONS}`, price_options).prop('checked')
            ? 1
            : 0,
        );
